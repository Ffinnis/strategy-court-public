...
| Public source | GitHub reported `Ffinnis/strategy-court-public` public, default branch `main`, license `MIT`. This confirms repository metadata, not that uncommitted additions are published. |
...
2. **Publish the verified build.** Review the existing working-tree additions together with these fixes. Preserve a dated commit record of the hackathon work. Apply migrations to the intended database, prepare the sample there, and release with no queued Court jobs. No Railway CLI or Railway token was available locally; the target deployment is identifiable through GitHub's deployment record. A source push or deployment has not been attempted.
