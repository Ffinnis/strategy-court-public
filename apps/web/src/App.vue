...
<template>
  ...
  <div class="app-shell">
    <header class="topbar" :class="{ 'topbar--workspace': route.name === 'case', 'topbar--auth': isAuthPage, 'topbar--landing': isLandingPage }">
      ...
      <nav v-if="isLandingPage" class="landing-nav" aria-label="Overview">
      ...
      <div class="topbar__meta">
        ...
        <template v-else>
          <span v-if="!isLandingPage" class="system-chip"><span class="system-chip__dot" /> 7 robustness tests</span>
          ...
          <RouterLink v-else class="button button--quiet button--small account-sign-in" :to="{ name: 'auth' }">Sign in</RouterLink>
        </template>
      </div>
    </header>
...
.topbar--landing {
  ...
  gap: 24px;
  height: 76px;
  padding-inline: max(var(--workspace-gutter), calc((100% - var(--workspace-shell)) / 2));
  box-shadow: none;
...
@media (max-width: 960px) {
  .topbar--landing { display: flex; height: 64px; gap: 16px; }
  .landing-nav { display: none; }
}

@media (max-width: 520px) {
  .topbar--landing .account-sign-in { display: inline-flex; }
}
