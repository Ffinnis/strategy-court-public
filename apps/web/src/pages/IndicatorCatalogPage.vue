...
async function createIndicator(): Promise<void> {
  ...
  try {
    ...
    notifications.push("Custom indicator created.");
...
<template>
  <div class="indicator-page">
    ...
    <section v-motion-reveal class="catalog-summary" aria-label="Indicator catalog summary">
    ...
    <div class="indicator-layout">
      <section class="catalog-panel" aria-labelledby="catalog-title">
        ...
        <div class="catalog-controls">
          ...
          <SegmentedControl :model-value="filter" label="Filter indicators" :options="[{value:'all',label:'All'},{value:'ready',label:'Ready'},{value:'planned',label:'Planned'},{value:'custom',label:'Custom'}]" @update:model-value="filter=$event as CatalogFilter" />
        </div>
        ...
        <div v-else class="catalog-list">
          <div v-for="indicator in visibleIndicators" :key="indicator.id" class="catalog-item"><RouterLink class="catalog-row" :to="`/indicator/${encodeURIComponent(indicator.id)}`">
          ...
          </RouterLink><ContextPreview :title="indicator.name" :description="indicator.description" :facts="[{label:'Version',value:String(indicator.version ?? 'Built-in')},{label:'Parameters',value:indicator.parameters?.map(parameter=>`${parameter.label}: ${parameter.default}`).join(', ') || 'None'},{label:'Sources',value:indicator.allowedSources?.join(', ') || 'See definition'}]" :to="`/indicator/${encodeURIComponent(indicator.id)}`" /></div>
        </div>
      </section>
      ...
      <section class="builder-panel" aria-labelledby="builder-title">
        ...
        <form novalidate class="builder-form" @submit.prevent="createIndicator">
          <div class="field-grid">
            <label class="field"><span>Name</span><input v-model="form.name" :aria-invalid="attempted && !form.name.trim()" aria-describedby="indicator-validation" class="input" maxlength="120" required placeholder="RSI threshold signal" /></label>
            <label class="field">
              <span>Sharing</span>
              <FormSelect id="indicator-sharing" v-model="form.sharingState" :options="sharingOptions" aria-label="Sharing" />
            </label>
          </div>
          <label class="field"><span>Description</span><textarea v-model="form.description" :aria-invalid="attempted && !form.description.trim()" aria-describedby="indicator-validation" class="input" maxlength="2000" required placeholder="What the indicator measures and when it should be used." /></label>
          ...
          <div v-if="createError" id="indicator-validation" class="inline-state inline-state--error" role="alert">{{ createError }}</div>
          ...
          <div class="form-actions">
            ...
            <button class="button" type="submit" :disabled="creating || loading">
...
.catalog-item{display:grid;grid-template-columns:minmax(0,1fr) 36px;align-items:center;border-bottom:1px solid #282828;}
.catalog-item .catalog-row{border-bottom:0;border-top:0;}
.catalog-item .catalog-row:hover{transform:none;border-radius:0;padding-inline:4px;}
