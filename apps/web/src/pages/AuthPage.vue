...
async function submit() {
  ...
  if (!validName || !validEmail || form.password.length < 8) return;
    ...
    <section class="auth-entry" aria-labelledby="auth-title">
      <div class="auth-form-wrap">
        ...
        <form novalidate @submit.prevent="submit">
          ...
          <div v-if="error" class="auth-message auth-message--error" role="alert">
            <AlertCircle :size="16" />
            <span>{{ error }}</span>
          </div>
...
@media(max-width:940px){.auth-page{grid-template-columns:1fr}.auth-context{min-height:auto;padding:72px 32px 56px;border-right:0;border-bottom:1px solid rgba(255,255,255,.06)}.auth-context h1{max-width:720px}.case-glimpse{margin-top:46px}.auth-entry{padding-block:72px}}
