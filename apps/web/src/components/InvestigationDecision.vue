...
async function save(confirm: boolean) {
  ...
  catch(error) {validationError.value=error instanceof Error ? error.message : "Review the decision fields.";return;}
...
async function inspect(reference: EvidenceReference) {
  if (reference.kind === "verdict") {
    ...
    await nextTick();
    document.getElementById(`verdict-${reference.id}`)?.scrollIntoView({block:"center"});
    return;
  }
...
<template>
  <section class="investigation-decision" aria-labelledby="decision-heading">
    <header class="decision-heading">
      <div><h3 id="decision-heading">{{ guidance.title }}</h3><p>{{ guidance.detail }}</p></div>
    ...
    <div v-if="store.recordedDecision && !editing" class="recorded-decision">
      ...
      <button class="button button--secondary" type="button" @click="store.activeTab='audit'">Report and audit</button>
    ...
    <form v-if="editing" class="decision-form" @submit.prevent="save(true)">
      ...
      <label>Why this conclusion?<textarea v-model="fields.rationale" rows="3" maxlength="2000" required /></label>
      <fieldset><legend>Supporting evidence, choose one to five</legend><div class="citation-options"><label v-for="reference in citations" :key="`${reference.kind}:${reference.id}`"><input type="checkbox" :checked="cited(reference)" :disabled="!cited(reference) && fields.evidenceRefs.length>=5" @change="toggleCitation(reference)"><span>{{ reference.label }}</span><small>{{ reference.kind }}</small></label></div></fieldset>
      <label>What remains uncertain?<textarea v-model="fields.uncertainties" rows="2" maxlength="2000" placeholder="State the evidence limits, or explicitly write None identified." required /></label>
      <label>What would justify revisiting this?<textarea v-model="fields.revisitCriteria" rows="2" maxlength="2000" required /></label>
      ...
      <p v-if="validationError || store.decisionError" class="decision-error" role="alert">{{ validationError || store.decisionError }}</p>
